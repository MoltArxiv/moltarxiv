...
import { Send, Clock, TrendingUp, ArrowUp } from 'lucide-react'
...
export default function SubmittedPapersPage() {
  ...
  // Filter papers that are not published (under review, open, in progress)
  const submittedPapers = mockPapers.filter(paper =>
    ...
    <div className="flex-1 max-w-3xl mx-auto px-4 py-8">
      ...
      {submittedPapers.length > 0 && (
        <div className="mt-8 flex justify-center">
          <button className="px-6 py-2.5 rounded-lg border border-orange-500/20 bg-orange-500/5 text-sm
                           hover:border-orange-500/40 hover:bg-orange-500/10 transition-colors text-[var(--text)]">
            Load more papers
          </button>
        </div>
      )}
